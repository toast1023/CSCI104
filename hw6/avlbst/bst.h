...
protected:
    ...
    bool isBalancedRoot(std::shared_ptr<Node<Key, Value>> root) const;
...
{
    // TODO
    auto sp = std::make_shared<std::pair<const Key,Value>>(current_->getItem());
    return sp;
}
...
{
    // TODO
    bool isTrue = (this->current_ == rhs.current_);
    return isTrue;
}
...
{
    // TODO
    //post-order traversal helper function to delete
    clear();
}
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    {
        //NEED PARENT NODE
        auto newNode=std::make_shared<std::pair<const Key,Value>>(keyValuePair.first, keyValuePair.second, nullptr);
    }
    else
    {
        std::shared_ptr<Node<Key, Value>> myNode(root_);
        if(keyValuePair.first < myNode->getKey())
        {
            if(myNode->getLeft() ==nullptr)
            {
                //why won't it take parent node?
                auto newNode=std::make_shared<std::pair<const Key,Value>>(keyValuePair.first, keyValuePair.second, newNode);
                myNode->setLeft(newNode);
                return;
            }
            else
            {
...
void BinarySearchTree<Key, Value>::clearHelper(std::shared_ptr<Node<Key, Value>> root)
{
    ...
    {
        //go left and right and delete everything
...
std::shared_ptr<Node<Key, Value>> BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    auto walkNode=std::make_shared<std::pair<const Key,Value>>(root_);
    if(walkNode != nullptr)
    {
        //REACHED LEFT MOST NODE
        while(walkNode->getLeft() != nullptr)
        {
            walkNode = walkNode->getLeft();
        }
        return walkNode;
    }
    else 
...
{
    // TODO
    return isBalancedRoot(root_);
}
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isBalancedRoot(std::shared_ptr<Node<Key, Value>> root) const
{
    //FROM LAB
    bool left = false;
    bool right = false;
