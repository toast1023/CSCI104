...
PlaceResult Board::test_place(const Move& move) const {
    ...
    //get position of first move position where placing tiles
   	...
   	for (unsigned int i =0; i <(move.tiles.size()); i++)
   	{
   		...
   		{
        //COME BACK AND FIX IN CASE OF '?'
        if(this->at(current).get_tile_kind().letter == '?')
        {
          newWord = newWord+ this->at(current).get_tile_kind().assigned;
        }
        else
        {
...
bool Board::is_anchor_spot(Position p) const
{
  ...
  //------ p does not have tile------
  //check start square
  else if (p == start)
  {
    if (in_bounds_and_has_tile(start)==false)
    {
      return true; 
    }
  }
  //check left right top down for adjacent tiles
  else if(in_bounds_and_has_tile(Position(p.row,p.column-1)))
  {
    return true;
  }
  else if(in_bounds_and_has_tile(Position(p.row-1,p.column)))
  {
    return true; 
  }
  else if (in_bounds_and_has_tile(Position(p.row,p.column+1)))
...
vector<Board::Anchor> Board::get_anchors() const
{
  ...
  //iterate through every square 2 for loops
  //when you find an anchor start looping backwords until you hit another anchor or wall
  ...
  for(unsigned int i=0; i<rows; i++)
  {
    for(unsigned int j=0; j<columns; j++)
    {
      ...
      //check LEFT (opposite direction)
      if(is_anchor_spot(current))
      {
        //runs until you hit end
        for (int current=j-1; current >= 0 ; current--)
        {
          //exit if you hit an anchor or tile
          if(is_anchor_spot(Position(i,current)) || in_bounds_and_has_tile(Position(i,current)))
          {
            break;
          }
          //make sure in bounds else break
          else if (is_in_bounds(Position(i,current)))
          {
            currLimit++;
          }
          else
        ...
        result.push_back(Anchor(current,across,currLimit));
        currLimit = 0;
      }
      //same code as above but in different direction
      //check UP (opposite direction)
      if(is_anchor_spot(current))
      {
        //runs until you hit end
        ...
        {
          //exit if you hit an anchor or tile
          if(is_anchor_spot(Position(current,j)) || in_bounds_and_has_tile(Position(current,j)))
          {
            break;
          }
          //make sure in bounds else break
          else if(is_in_bounds(Position(current,j)))
          {
            currLimit++;
          }
          else 
        ...
        result.push_back(Anchor(current,down,currLimit));
        currLimit = 0;
      }
    }
  }
